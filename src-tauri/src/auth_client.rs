...
pub async fn visitor_login(phone: &str, code: &str) -> Result<String, String> {
    ...
    let response = reqwest::Client::new()
        ...
        .header("Referer", "https://iw68lh.aliwork.com/o/fk_login_app")
        .json(&body)
        .send()
        .await
    ...
    let ac_token = json
        .pointer("/data/acToken")
        .and_then(Value::as_str)
        .ok_or_else(|| format!("visitorLogin: missing data.acToken in response: {text}"))?;

    Ok(ac_token.to_string())
}
